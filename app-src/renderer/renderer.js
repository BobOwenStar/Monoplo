...
// --- Update banner --------------------------------------------------------
//
// Only shown for states the user might actually need to act on or know
// about (downloading, ready-to-restart, or a failed check) — routine
// "checking for updates" is silent, same philosophy as the status box:
// say something only when there's something worth saying.
...
function renderUpdateBanner(status) {
  ...
  if (!status || status.state === 'not-available') {
    banner.hidden = true;
    return;
  }

  banner.hidden = false;
  restartBtn.hidden = status.state !== 'ready';

  if (status.state === 'downloading') {
    text.textContent = `Downloading update${status.version ? ` v${status.version}` : ''}… ${status.percent || 0}%`;
  } else if (status.state === 'ready') {
    text.textContent = `Update v${status.version} ready — restart to apply.`;
